...
(Note that terminated threads will be removed from the group as soon as possible
so they don't leak space.)
-}
...
fork doFork (ThreadGroup mv) act = do
  ...
  where
    deleteMyTid = do
      ...
      purelyModifyMVar mv $ deleteFirstWhich' ((nativeTid ≡) ∘ threadId)
